...
const oldProblemItem = `function ProblemItem({ prob, idx, activeIndex }: { prob: any, idx: number, activeIndex: number }) {
  ...
  // Apenas o scroll define o foco agora (sem hover state)
  ...
  return (
    <motion.div 
      ...
      className={\`group relative border-b border-white/[0.06] last:border-0 py-12 md:py-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-12 transition-all duration-700 -mx-6 px-6 sm:mx-0 sm:px-8 rounded-2xl md:rounded-none \${isActive ? 'bg-white/[0.025]' : 'opacity-40'}\`}
...
const newProblemItem = `function ProblemItem({ prob, idx, activeIndex }: { prob: any, idx: number, activeIndex: number }) {
  ...
  // Apenas o scroll define o foco agora
  const isActive = activeIndex === idx;
  ...
  return (
    <motion.div 
      ...
      initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }}
      variants={{ hidden: { opacity: 0, y: 40 }, visible: { opacity: 1, y: 0, transition: { duration: 0.7, delay: idx * 0.15, ease: [0.21, 0.47, 0.32, 0.98] } } }}
      className={\`group relative border-b border-white/[0.06] last:border-0 py-12 md:py-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-12 transition-all duration-700 -mx-6 px-6 sm:mx-0 sm:px-8 rounded-2xl md:rounded-none \${isActive ? 'bg-white/[0.01]' : 'opacity-30 grayscale-[50%]'}\`}
      data-problem-item={idx}
    >
      <div className={\`absolute left-0 w-[2px] bg-gradient-to-b from-transparent via-[#C9A84C] to-transparent transition-all duration-700 ease-in-out \${isActive ? 'opacity-100 top-0 bottom-0' : 'opacity-0 top-1/4 bottom-1/4'}\`} />
      ...
      <div className="w-full md:w-[45%] flex flex-col relative z-10 pl-2 md:pl-4">
        <div className="flex items-center gap-4 mb-5 transition-opacity duration-500">
          <span className="text-[#C9A84C] font-serif italic text-lg">{prob.num}</span>
          <span className="h-[1px] w-12 bg-gradient-to-r from-[#C9A84C]/60 to-transparent"></span>
        </div>
        <h3 className="text-[26px] md:text-[32px] font-serif font-semibold text-white transition-colors duration-500 leading-tight tracking-tight">
          {prob.title}
        </h3>
      </div>
      
      <div className="w-full md:w-[45%] flex items-start relative z-10 pr-2 md:pr-4">
        <p className="text-[#9CA3AF] transition-colors duration-500 text-[16px] md:text-[17px] leading-relaxed w-full font-light">
          {prob.desc}
        </p>
      </div>
    </motion.div>
...
if (content.includes(oldProblemItem)) {
  ...
  console.log('Updated perfect single active scroll logic');
} else {
