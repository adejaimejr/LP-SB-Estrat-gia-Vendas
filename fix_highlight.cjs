...
const oldComponent = `function ProblemItem({ prob, idx }: { prob: any, idx: number }) {
  ...
  // Ativa quando o item está visível na tela (usando uma margem menor para que fique ativo logo que aparece bem)
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-[45%] flex flex-col relative z-10 pl-2 md:pl-4">
        <div className={\`flex items-center gap-4 mb-5 transition-opacity duration-500 \${isInView ? 'opacity-100' : 'opacity-50 group-hover:opacity-100'}\`}>
        ...
        <h3 className={\`text-[26px] md:text-[32px] font-serif font-semibold transition-colors duration-500 leading-tight tracking-tight \${isInView ? 'text-white' : 'text-white/50 group-hover:text-white'}\`}>
          {prob.title}
        </h3>
      </div>
      
      <div className="w-full md:w-[45%] flex items-start relative z-10 pr-2 md:pr-4">
        <p className={\`transition-colors duration-500 text-[16px] md:text-[17px] leading-relaxed w-full font-light \${isInView ? 'text-[#D1D5DB]' : 'text-[#666666] group-hover:text-[#D1D5DB]'}\`}>
          {prob.desc}
        </p>
      </div>
    </motion.div>
...
const newComponent = `function ProblemItem({ prob, idx }: { prob: any, idx: number }) {
  const ref = useRef(null);
  // Restringe bastante a margem para que apenas o item que estiver cruzando a exata linha central da tela fique ativo
...
if (content.includes(oldComponent)) {
  ...
  console.log('Fixed highlight behavior');
} else {
